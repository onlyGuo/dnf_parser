...
import com.xiaoyouma.dnf.parser.pvf.model.PvfData;

import java.nio.ByteBuffer;
import java.nio.ByteOrder;
import java.util.List;

import static com.xiaoyouma.dnf.parser.util.BufferHelper.readBytes;
import static com.xiaoyouma.dnf.parser.util.ByteHelper.bytesToFloat;
...
public class UIParser implements IParser {
    ...
    public JSONObject convert(Pvf pvf, byte[] data) {
        return parse(pvf, null, data).getDict();
    }

    @Override
    public JSONObject convert(Pvf pvf, String path, byte[] data) {
        return parse(pvf, path, data).getDict();
    }

    @Override
    public String convertSource(Pvf pvf, byte[] data) {
        return parse(pvf, null, data).getSource();
    }

    @Override
    public String convertSource(Pvf pvf, String path, byte[] data) {
        return parse(pvf, path, data).getSource();
    }

    private PvfData parse(Pvf pvf, String path, byte[] data) {
        ...
        while (buffer.hasRemaining()) {
            if (buffer.remaining() >= 5) {
                ...
                switch (unitType) {
                    ...
                    case 10:
                        unitTypes.add(unitType);
                        values.add(pvf.getNString(path, pvf.getStringTable(bytesToInt(value))));
                        break;
                }
            } else {
                break;
